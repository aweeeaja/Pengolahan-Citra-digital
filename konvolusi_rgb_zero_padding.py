...
zero_padding = np.zeros((jml_baris_zp, jml_kolom_zp,3))

# buat matrix hasil
matriks_baru = np.zeros((jml_baris, jml_kolom,3))

# masukan citra awal ke matrix zero padding
for brs in range (jml_baris):
    for klm in range (jml_kolom):
        zero_padding [brs+1, klm+1] = gambar[brs, klm]

# looping pixel untuk konvolusi
for brs in range(1,jml_baris_zp-1):
    for klm in range(1,jml_kolom_zp-1):
        for ch in range (3):
        #baca dan kalikan matriks 3x3 dari gambar awal dengan kernel 3x3
            a = zero_padding[brs-1,klm-1,ch] * kernel[0,0]   #kiri atas
            b = zero_padding[brs-1,klm,ch] * kernel[0,1]     #tengah atas
            ...
            matriks_baru[brs-1, klm-1,ch] = konvolusi
